...
for index1, layer_1 in enumerate(nodes1):
    for index2, layer_2 in enumerate(nodes2):
        ...
        model_l2.train(train_inpf)
        dictionary = feature_imp(model_l2, test_df)
        #print('TEST RESULTS ', layer_1, layer_2)
        dictionary_auc = feature_auc(model_l2, test_df)
        #results = model_l2.evaluate(val_inpf)
        clear_output()
        features = list(dictionary['loss'].keys())[:-1] #iris['feature_names']
        importances = np.array(list(dictionary['loss'].values()))[:-1] #rnd_clf.feature_importances_
        indices = np.argsort(importances)

        plt.title('Feature Importances - loss', size = 15)
        plt.barh(range(len(indices)), importances[indices] * 100, color='b', align='center')
        plt.yticks(range(len(indices)), [features[i] for i in indices], size = 12.5)
        ax = plt.gca()
        ax.set_xscale('log')
        # plt.tick_params(axis='x', which='minor')

        ax.set_xticks(np.arange(100, 105, 1))
        ax.get_xaxis().set_major_formatter(matplotlib.ticker.PercentFormatter(decimals=0))
        # ax.xaxis.set_minor_formatter(FormatStrFormatter("%.1f"))
        # plt.xticks(np.linspace(1,1.11,10))
        plt.xlabel('Relative Importance', size = 15)
        # plt.xscale('log')
        plt.tight_layout()
        plt.savefig('feature_importance_loss.pdf', format='pdf', dpi=1000)
        plt.show()

        features = list(dictionary['average_loss'].keys())[:-1] #iris['feature_names']
        importances = np.array(list(dictionary['average_loss'].values()))[:-1] #rnd_clf.feature_importances_
        indices = np.argsort(importances)
        ...
        #plt.xscale('log')
        plt.tight_layout()
        plt.savefig('feature_importance_avg_loss.pdf', format='pdf', dpi=1000)
        plt.show()

        import matplotlib

        features = list(dictionary_auc['auc'].keys())[:-1] #iris['feature_names']
        importances = np.array(list(dictionary_auc['auc'].values()))[:-1] #rnd_clf.feature_importances_
        indices = np.argsort(importances)

        plt.title('Feature Importances - AUC', size = 15)
        plt.barh(range(len(indices)), importances[indices]*100, color='b', align='center')
        plt.yticks(range(len(indices)), [features[i] for i in indices], size = 12.5)
        ax = plt.gca()
        ax.set_xscale('log')
        ...
        ax.set_xticks(np.arange(100,111,2))
        ax.get_xaxis().set_major_formatter(matplotlib.ticker.PercentFormatter(decimals = 0))
        #ax.xaxis.set_minor_formatter(FormatStrFormatter("%.1f"))
        #plt.xticks(np.linspace(1,1.11,10))
        plt.xlabel('Relative Importance', size = 15)
        #plt.xscale('log')
        plt.tight_layout()
        plt.savefig('feature_importance_AUC_new.png', format='png', dpi=1000)
        plt.show()

        features = list(dictionary_auc['recall'].keys())[:-1] #iris['feature_names']
        importances = np.array(list(dictionary_auc['recall'].values()))[:-1] #rnd_clf.feature_importances_
        indices = np.argsort(importances)

        plt.title('Feature Importances - RECALL', size = 15)
        plt.barh(range(len(indices)), importances[indices]*100, color='b', align='center')
        plt.yticks(range(len(indices)), [features[i] for i in indices], size = 12.5)
        ax = plt.gca()
        ax.set_xscale('log')
        ...
        #plt.xscale('log')
        plt.tight_layout()
        plt.savefig('feature_importance_RECALL_new.png', format='png', dpi=1000)
        plt.show()